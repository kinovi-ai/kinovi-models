#!/usr/bin/env bash
# video-edit.sh — Edit an existing clip with Seedance 2.5 Video Edit on Kinovi: keep its motion and framing, change what is in it.
#
# Usage:
...
{
  "prompt": "Keep the same hand, the same slow hand movements and the same camera framing, but change the nail polish to glossy black with fine gold flakes and change the grey ribbed sweater to a deep burgundy one. Everything else stays identical.",
