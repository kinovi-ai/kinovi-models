#!/usr/bin/env bash
# video-extend.sh — Continue an existing clip with Seedance 2.5 Video Extend on Kinovi: pick up from its last frame and generate what happens next.
#
# Usage:
...
{
  "prompt": "The hand with the colorful nails slowly turns palm-up, the fingers spread, then the hand reaches down and picks up a small gold ring from the table. Same lighting, same grey sweater, same wall.",
