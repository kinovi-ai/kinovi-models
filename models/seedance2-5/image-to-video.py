#!/usr/bin/env python3
# image-to-video.py — Animate a still image with Seedance 2.5 on Kinovi.
#
# Usage:
...
for i, item in enumerate(info["output"]):
    ...
    filename = f"image-to-video-{i}{ext}" if len(info["output"]) > 1 else f"image-to-video{ext}"
