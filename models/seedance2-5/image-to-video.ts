// image-to-video.ts — Animate a still image with Seedance 2.5 on Kinovi.
//
// Usage:
//   Put KINOVI_API_KEY in a .env file (repo root or this folder), or:
//   export KINOVI_API_KEY=your-api-key     # https://kinovi.ai/api-keys
//   npx tsx image-to-video.ts             # or: node --experimental-strip-types image-to-video.ts
//
...
const INPUTS = {
  prompt:
    "Bring this image to life: a slow push-in on the subject, gentle wind moving the " +
    "details, soft natural light shifting across the scene.",
  // One image = opening frame. Two images = first and last frame.
  // For 3-30 reference images add "mode": "reference". Replace with your own URLs.
  imageUrls: ["https://static.kinovi.ai/materials/20260705/1783247738434-3a5b9fbb.png"],
  duration: 5, // 4-30 seconds, billed per second
  outputResolution: "720p", // 480p | 720p | 1080p
  // The output keeps the image's aspect ratio; aspectRatio is not applied to image-to-video.
};
...
async function main() {
  ...
  for (const [i, item] of items.entries()) {
    ...
    const filename = items.length > 1 ? `image-to-video-${i}${ext}` : `image-to-video${ext}`;
