<h1 align="center">Seedance 2.0 Mini</h1>

<p align="center">
  The smallest, cheapest Seedance 2.0 variant on the Kinovi API — for drafts, batch tests and prompt iteration. Same inputs as Seedance 2.0 (text, keyframe images, up to 9 reference images, 3 videos and 3 audio clips) at 480p or 720p.<br>
  4–15 second clips with a generated audio track.
</p>

<p align="center">
  <img alt="Model id" src="https://img.shields.io/badge/model-seedance2.0--mini-6366f1?style=flat-square">
...
**Full docs** [kinovi.ai/docs/models](https://kinovi.ai/docs/models) &nbsp;·&nbsp; **Try it** [Playground](https://kinovi.ai/app/gallery?model=seedance2.0-mini) &nbsp;·&nbsp; **API key** [kinovi.ai/app/api-keys](https://kinovi.ai/app/api-keys) &nbsp;·&nbsp; **Content policy** [kinovi.ai/terms](https://kinovi.ai/terms)
...
| `400` | `{ "message": "Unknown model" }` | `model` is not exactly `"seedance2.0-mini"` (`"seedance2-mini"` is rejected). |
| `401` | `{ "message": "Invalid API Key" }` | Missing or wrong `Authorization` header. |
| `402` | `{ "message": "Insufficient credits", "required": 100, "available": 0 }` | Not enough credits for this duration × resolution. |
...
|:--|:--|:--|:--|
| `model` | `string` | **required** | Must be `"seedance2.0-mini"`. |
...
Poll every couple of seconds until `status` is `success` or `fail`. A short clip typically takes 2–4 minutes at `480p` and 2–6 minutes at `720p`, occasionally up to 10 minutes. Poll for at least 10 minutes before treating a task as stuck.
...
Priced **per second of video**, by `outputResolution`. Attaching a reference video (`videoUrls`) moves the task to that resolution's reference tier. Reference images and audio do not change the price. The full `duration` is charged at submit and is the final price; failed tasks are refunded in full. Prices above are the list rates; a 40% launch discount applies until Oct 8, 2026. Live prices: [kinovi.ai/models/seedance2-mini](https://kinovi.ai/models/seedance2-mini).

| | 480p | 720p |
|:--|--:|--:|
| **per second** | $0.0353 · 7.5 cr | $0.0942 · 20 cr |
| **per second, with reference video** | $0.0447 · 9.5 cr | $0.1130 · 24 cr |

Examples: the text-to-video, image-to-video, first-last-frame-to-video and omni-reference-to-video scripts generate 5 s at `720p` (**100 credits · $0.47**); reference-to-video and video-extend also use 5 s at `720p` but include a reference video (**120 credits · $0.57**); video-edit matches its 10 s source clip (**240 credits · $1.13**).

<br>

<p align="center">
  <a href="../../README.md">← All models</a> &nbsp;·&nbsp;
  <a href="https://kinovi.ai/models/seedance2-mini">Model page</a> &nbsp;·&nbsp;
  <a href="https://kinovi.ai/app/gallery?model=seedance2.0-mini">Playground</a>
</p>
