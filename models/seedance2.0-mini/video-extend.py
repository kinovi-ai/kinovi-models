#!/usr/bin/env python3
# video-extend.py — Continue an existing clip with Seedance 2.0 Mini on Kinovi: pick up from its last frame and generate what happens next.
#
# Usage:
...
INPUTS = {
    "prompt": (
        "Continue this video from its final frame. The hand with the colorful nails slowly "
        "turns palm-up, the fingers spread, then the hand reaches down and picks up a small "
        "gold ring from the table. Same lighting, same grey sweater, same wall."
    ),
    # The clip to continue goes in videoUrls (up to 15 s). Start the prompt with
    # "Continue this video from its final frame" and describe the new action.
    "videoUrls": [
        "https://static.kinovi.ai/videos/seedance_video_1775631198994_17cd129c.mp4",
    ],
    "mode": "reference",
    "duration": 5,                # length of the NEW footage; 4-15 seconds, billed per second
...
for i, item in enumerate(info["output"]):
    ...
    filename = f"video-extend-{i}{ext}" if len(info["output"]) > 1 else f"video-extend{ext}"
