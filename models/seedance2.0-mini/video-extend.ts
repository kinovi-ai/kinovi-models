// video-extend.ts — Continue an existing clip with Seedance 2.0 Mini on Kinovi: pick up from its last frame and generate what happens next.
//
// Usage:
...
async function main() {
  ...
  for (const [i, item] of items.entries()) {
    ...
    const filename = items.length > 1 ? `video-extend-${i}${ext}` : `video-extend${ext}`;
