<h1 align="center">Wan 3.0 Prime Reference-to-Video</h1>

<p align="center">
  Wan 3.0 Prime reference-to-video on the Kinovi API — the faster Wan 3.0 tier. Build a shot from up to 10 reference images, 5 reference videos and 5 audio clips, cited in the prompt as <code>@Image1</code>, <code>@Video1</code>, <code>@Audio1</code>.<br>
  2–30 second clips with a generated audio track, 480p to 1080p.
</p>

<p align="center">
  <img alt="Model id" src="https://img.shields.io/badge/model-wan3.0--prime--ref--to--video-6366f1?style=flat-square">
  ...
  <a href="https://kinovi.ai/models/wan3-prime-ref-to-video"><img alt="Model page" src="https://img.shields.io/badge/kinovi.ai-model%20page-111827?style=flat-square"></a>
</p>

> Try it in the [Playground](https://kinovi.ai/app/gallery?model=wan3.0-prime-ref-to-video) · Full reference at [kinovi.ai/docs/models/wan3.0-prime-ref-to-video](https://kinovi.ai/docs/models/wan3.0-prime-ref-to-video) · Get an [API key](https://kinovi.ai/app/api-keys)
...
```json
{
  "model": "wan3.0-prime-ref-to-video",
  "inputs": {
...
```json
{
  "taskId": "task_lqdkkitat9m6lo2us25sq21x"
}
```
...
|:--|:--|:--|:--|
| `model` | `string` | **required** | Must be `"wan3.0-prime-ref-to-video"`. |
...
Wan 3.0 has three faces — text-to-video, image-to-video and reference-to-video — each in a standard and a Prime tier. [`wan3.0-ref-to-video`](../wan3.0-ref-to-video/README.md) takes the same inputs at a lower per-second price and roughly twice the wait. Switching is a one-line change of `model`. The other Wan 3.0 Prime faces: [`wan3.0-prime-text-to-video`](../wan3.0-prime-text-to-video/README.md) · [`wan3.0-prime-image-to-video`](../wan3.0-prime-image-to-video/README.md).
...
Poll every few seconds until `status` is `success` or `fail`. A 5-second `720p` clip usually takes 2–3 minutes. Longer clips and `1080p` take longer; poll for at least 15 minutes before treating a task as stuck.
...
```json
{
  ...
  "output": [
    {
      "url": "https://static.kinovi.ai/videos/wan3_1790101525091_99f12fec.mp4",
      "width": 1280,
      "height": 720,
      "mediaType": "video/mp4"
    }
  ...
  "error": null,
  "createTime": 1790101395538,
  "completeTime": 1790101528294
}
```
...
Priced **per second of video**, by `outputResolution`. Reference images, videos and audio do not change the price; only the output length counts. `duration` × the rate is charged at submit; failed tasks are refunded in full. Live prices: [kinovi.ai/models/wan3-prime-ref-to-video](https://kinovi.ai/models/wan3-prime-ref-to-video).

| | 480p | 720p | 1080p |
|:--|--:|--:|--:|
| **per second** | $0.0877 · 18.86 cr | $0.1806 · 38.83 cr | $0.3611 · 77.66 cr |

Example: the `omni-reference-to-video` and `reference-to-video` scripts generate 5 s at `720p` — **194.15 credits · $0.90**.

<br>

<p align="center">
  <a href="../../README.md">← All models</a> &nbsp;·&nbsp;
  <a href="https://kinovi.ai/models/wan3-prime-ref-to-video">Model page</a> &nbsp;·&nbsp;
  <a href="https://kinovi.ai/app/gallery?model=wan3.0-prime-ref-to-video">Playground</a>
</p>
