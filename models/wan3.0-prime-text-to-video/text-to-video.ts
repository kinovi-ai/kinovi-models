// text-to-video.ts — Generate a video from a text prompt with Wan 3.0 Prime on Kinovi.
//
// Usage:
//   Put KINOVI_API_KEY in a .env file (repo root or this folder), or:
//   export KINOVI_API_KEY=your-api-key     # https://kinovi.ai/api-keys
//   npx tsx text-to-video.ts             # or: node --experimental-strip-types text-to-video.ts
//
...
const MODEL = "wan3.0-prime-text-to-video";
const INPUTS = {
  prompt:
    "A golden retriever running through autumn leaves in slow motion, cinematic " +
    "depth of field, warm afternoon light.",
...
async function main() {
  ...
  for (const [i, item] of items.entries()) {
    ...
    const filename = items.length > 1 ? `text-to-video-${i}${ext}` : `text-to-video${ext}`;
